...
rem You can find description of all Intel(R) MPI Library parameters in the
rem Intel(R) MPI Library Reference Manual.
set I_MPI_DAPL_DIRECT_COPY_THRESHOLD=655360

rem     "export I_MPI_PIN_CELL=core"
rem     You can use this variable (beginning Intel(R) MPI Library 4.0.1) for cases
rem     if HT is enabled.
...
rem Environment variables can also be set on the Intel(R) MPI Library command
rem line using the -genv option (to appear before the -np 1):
echo.   >> %OUT%
