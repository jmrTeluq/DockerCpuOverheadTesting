...
if [ ${INTEL_TARGET_ARCH} = "ia32" ]; then
  INTEL_TARGET_ARCH_IA32="ia32"; export INTEL_TARGET_ARCH_IA32
else
  unset INTEL_TARGET_ARCH_IA32
fi

# Compiler driver and other executables are in intel64 (host), include ia32/intel64 targets.
PATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/bin/intel64:${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/bin" "${PATH:-}") ; export PATH
if [ ${USE_INTEL_LLVM} = "1" ]; then
  PATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/bin-llvm" "${PATH:-}") ; export PATH
fi

if [ ${INTEL_TARGET_ARCH} = "ia32" ]; then
  LIBRARY_PATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/compiler/lib/ia32_lin:${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/lib" "${LIBRARY_PATH:-}") ; export LIBRARY_PATH
else
  LIBRARY_PATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/compiler/lib/intel64_lin:${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/lib" "${LIBRARY_PATH:-}") ; export LIBRARY_PATH
fi
...
PKG_CONFIG_PATH=$(prepend_path "${CMPLR_ROOT}/lib/pkgconfig" "${PKG_CONFIG_PATH:-}"); export PKG_CONFIG_PATH

CMAKE_PREFIX_PATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/IntelDPCPP" "${CMAKE_PREFIX_PATH:-}"); export CMAKE_PREFIX_PATH

NLSPATH=$(prepend_path "${CMPLR_ROOT}/${INTEL_TARGET_PLATFORM}/compiler/lib/intel64_lin/locale/%l_%t/%N" "${NLSPATH:-}") ; export NLSPATH
