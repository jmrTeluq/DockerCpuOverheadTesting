...
// TODO: Put the FLOP count, Memory BW and Memory Usage models into separate functions
    ...
    OutputFile doc(fileName, "V3.1", "", params.yamlFileName);
    ...
    //varied domain dimensions for z-dimension here
    if (A.geom->npartz > 1) {
        doc.get("Local Domain Dimensions")->add("pz", A.geom->pz);
        doc.get("Local Domain Dimensions")->add("zl", A.geom->partz_nz[0]);
        doc.get("Local Domain Dimensions")->add("zu", A.geom->partz_nz[1]);
    }
  ...
  doc.get("User Optimization Overheads")->add("Optimization phase time vs reference SpMV+MG time", times[7]/times[8]);
    ...
    if (isValidRun) {
        ...
        doc.get(" Final Summary ")->add("    HPCG 2.4 Rating (for historical value) is", totalGflops24);
